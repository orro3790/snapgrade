 ...
 */
function getValidPosition(nodeId) {
...
const groupSelectState = $state({
    selectedNodeIds: new Set(),
    isGroupMode: false,
    generation: 0
});
const dragSelectState = $state({
    isDragging: false,
    isSelected: false,
    startNodeId: null,
    endNodeId: null,
 ...
 */
function groupNodesByParagraph(nodes) {
    const paragraphs = [];
 ...
 */
function setDocument(content, name) {
 ...
 */
function loadSerializedContent(serialized) {
 ...
 */
function updateNode(nodeId, text, correctionData, spacerData, type = 'normal', structuralRole) {
    const nodeIndex = editorState.nodes.findIndex((n) => n.id === nodeId);
    if (nodeIndex === -1)
        return;
 ...
 */
function toggleDeletion(nodeId) {
    const nodeIndex = editorState.nodes.findIndex((n) => n.id === nodeId);
    if (nodeIndex === -1)
        return;
 ...
 */
function insertNodeAfter(nodeId, text, type = 'normal') {
    const nodeIndex = editorState.nodes.findIndex((n) => n.id === nodeId);
    if (nodeIndex === -1)
        return;
    // Save current state for undo
 ...
 */
function getContent() {
    return editorState.nodes.reduce((acc, node) => acc + node.text, '');
}
/**
 ...
 */
function startDragSelection(nodeId) {
 ...
 */
function updateDragSelection(nodeId) {
 ...
 */
function isNodeInGroupSelection(nodeId) {
 ...
 */
function unpackGroupNode(groupNode) {
    ...
    return groupNode.metadata.groupedNodes.map((node, index) => ({
        ...node,
 ...
 */
function createMultiNodeDeletion(nodeIds) {
    const selectedNodes = nodeIds
        .map(id => editorState.nodes.find((n) => n.id === id))
        .filter(Boolean);
    if (selectedNodes.length === 0)
        return;
    // Save current state for undo
    ...
    const newNodes = editorState.nodes.filter((n) => !nodeIds.includes(n.id));
 ...
 */
function createMultiNodeCorrection(nodeIds, correctedText, pattern, explanation) {
    const selectedNodes = nodeIds
        .map(id => editorState.nodes.find((n) => n.id === id))
        .filter(Boolean);
    if (selectedNodes.length === 0)
 ...
 */
function toggleMultiNodeDeletion(nodeIds) {
    ...
    if (validNodeIds.length === 1) {
        ...
        if (node) {
            updateNode(nodeId, node.text, node.correctionData, node.spacerData, node.type === 'deletion' ? 'normal' : 'deletion');
        }
        return;
    }
    // Check if any of the nodes are already group nodes (deletion or correction)
    const groupNode = editorState.nodes.find(n => validNodeIds.includes(n.id) &&
    ...
    if (groupNode) {
        ...
        const insertIndex = newNodes.findIndex(n => n.metadata.position > groupNode.metadata.position);
        if (insertIndex === -1) {
...
function removeNodes(nodeIds) {
    ...
    editorState.undoStack = [...editorState.undoStack, editorState.nodes];
    editorState.redoStack = [];
    updateNodes(editorState.nodes.filter((node) => !nodeIds.includes(node.id)));
}
/**
...
export const editorStore = {
    ...
    get groupSelect() {
        return {
            ...
            isNodeSelected: (nodeId) => isNodeInGroupSelection(nodeId)
        };
    },
    /**
