...
const commandStore = {
     ...
     */
    isActive: (commandId: string) => commandState.activeCommands.has(commandId),
 ...
 */
export function getCommandState(commandId: string) {
    return {
...
export const EditorCommands = {
    ...
    DELETE: {
        ...
        execute: (nodeId: string) => {
            if (!EditorCommands.DELETE.canExecute(nodeId)) return;
            ...
            if (node.metadata.groupedNodes?.length) {
                if (node.type === 'deletion') {
                    ...
                    editorStore.updateNode(
                        nodeId,
                        node.text,
                        undefined, // Clear correction data
                        undefined,
                        'deletion'
                    );
                } else {
    ...
    UNPACK_GROUP: {
        id: 'unpack-group',
        execute: (nodeId: string) => {
            if (!EditorCommands.UNPACK_GROUP.canExecute(nodeId)) return;
            ...
            const newNodes = editorStore.nodes.filter(n => n.id !== nodeId);
            const insertIndex = newNodes.findIndex(n => 
                n.metadata.position > node.metadata.position
            );
    ...
    REMOVE: {
        id: 'remove',
        execute: (nodeId: string) => {
            if (!EditorCommands.REMOVE.canExecute(nodeId)) return;
    ...
    GROUP_REMOVE: {
        id: 'group-remove',
        execute: (nodeIds: string[]) => {
            if (!EditorCommands.GROUP_REMOVE.canExecute(nodeIds)) return;
    ...
    GROUP_UPDATE: {
        id: 'group-update',
        execute: (nodeIds: string[]) => {
            if (!EditorCommands.GROUP_UPDATE.canExecute(nodeIds)) return;
    ...
    INSERT_EMPTY: {
        id: 'insert-empty',
        execute: (nodeId: string) => {
            if (!EditorCommands.INSERT_EMPTY.canExecute(nodeId)) return;
    ...
    GROUP_CORRECTION: {
        id: 'group-correction',
        execute: ([nodeIds, correctedText]: [string[], string]) => {
            if (!EditorCommands.GROUP_CORRECTION.canExecute(nodeIds)) return;
    ...
    GROUP_DELETE: {
        id: 'group-delete',
        execute: (nodeIds: string[]) => {
            if (!EditorCommands.GROUP_DELETE.canExecute(nodeIds)) return;
