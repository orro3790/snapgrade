...
import { createSessionCookie, setSessionCookie, logout } from "$lib/server/auth";
...
export const actions: Actions = {
  login: async ({ request, cookies }) => {
    ...
    try {
      ...
      // First sign in with email and password using Firebase Auth
      const auth = getAuth();
  ...
  logout: async ({ cookies, locals }) => {
    ...
    await logout(cookies, locals.user.id);
    return { success: true };
  }
};