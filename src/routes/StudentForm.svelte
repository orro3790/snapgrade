...
<script lang="ts">
	...
	// Props
	let { data, selectedClass, onCancel } = $props<{
	...
	// Set the classId when creating a new student
	$effect(() => {
		if (!$form.metadata?.id && selectedClass) {
			$form.classId = selectedClass.metadata.id;
		}
	});
</script>
...
<div class="form-container" onkeydown={handleKeydown}>
	<div class="form-header">
		<h2>{$form.metadata?.id ? 'Edit' : 'New'} Student</h2>
		<button type="button" class="close-button" onclick={onCancel} aria-label="Cancel"> × </button>
	</div>

	<form method="POST" action="?/manageStudent" use:enhance={handleSubmit}>
		{#if $form.metadata?.id}
		...
		<div class="form-actions">
			...
			<button type="submit" class="submit-button" disabled={isSubmitting}>
				...
				{:else}
					{$form.metadata?.id ? 'Save Changes' : 'Add Student'}
				{/if}
			</button>
		</div>
	</form>
